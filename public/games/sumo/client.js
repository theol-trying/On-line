...
export default (function () {
  ...
  function applyColors() { CC = PAL[A.palette] || PAL.normal; TEAMCC = TEAMPAL[A.palette] || TEAMPAL.normal; FX = A.reduceFx ? 0 : 1; decorKey = ''; bodyCache = {}; }
  ...
  function showEndscreen(m) {
    ...
    endEl.innerHTML = `<div class="etitle" style="color:${champ ? colSeat(champ.seat) : K.washi}">${title}</div>
      <div class="emeta">⏱ ${m.stats.durationSec}s · ${m.stats.nParts} lutteurs</div>${mvpLine}<div class="elist">${rows}</div><div class="ehint">Espace / clic pour rejouer</div>`;
  }
  ...
  function onState(m) {
    ...
    if (m.round !== prevRound) { prevRound = m.round; buf = []; puffs.length = 0; salt.length = 0; waves.length = 0; flyers.length = 0; outTxt.length = 0; sparks.length = 0; }
    ...
    lastCount = m.count;
    if (m.sd && !prevSd) announceShrink();                // filet si l'événement 'shrink' s'est perdu (arrivée en cours de manche)
    prevSd = !!m.sd;
    prevGs = m.gs;
  ...
  function playFx(f) {
    ...
    if (f.type === 'dash') { psound('dash', f.x); dust(f.x, f.y, 10, 1.8); return; }
    if (f.type === 'brace') { psound('brace', f.x); dust(f.x, f.y, 8, 1.2); if (!A.reduceFx) waves.push({ x: f.x, y: f.y, r0: PR, r1: PR * 2.4, born: now, life: 420, col: '217,194,122', lw: 4 }); return; }
    if (f.type === 'out') {
      psound('out', f.x); music.sting('kill');
      const p = snap && snap.players[f.seat];
      flyers.push({ seat: f.seat, x: f.x, y: f.y, dir: Math.atan2(f.y - AR / 2, f.x - AR / 2), a: p ? p.a : 0, r: p ? (p.r || PR) : PR, born: now });
    ...
    if (f.type === 'pickup') {
      ...
      if (!A.reduceFx && d) { const c = hexRgb(d.c); waves.push({ x: f.x, y: f.y, r0: 8, r1: 34, born: now, life: 380, col: c.join(','), lw: 3 }); }
      return;
    }
  ...
  // o : { scale, spin, alpha, dashing, brace, heavy, grip, boost, me, shadow }
  ...
  function draw() {
    ...
    if (snap) {
      ...
      // lutteurs
      const pv = viewPlayers(now), me = mySeat >= 0 ? snap.players[mySeat] : null;
      ...
      snap.players.forEach(p => {
        ...
        drawRikishi(p.seat, v.x, v.y, v.a, r, { dashing: p.dashing, brace: p.brace, heavy: p.heavy, grip: p.grip, boost: p.boost, me: p.seat === mySeat && !over }, now);
    ...
    for (let i = flyers.length - 1; i >= 0; i--) {
      ...
      if (A.reduceFx) { drawRikishi(f.seat, f.x, f.y, f.a, f.r, { alpha: 1 - t, shadow: false }, now); continue; }
      ...
      drawRikishi(f.seat, f.x + dx, f.y + dy, f.a, f.r, { scale: 1 + 0.9 * e, spin: t * 9, alpha: 1 - t * t, shadow: false }, now);
    }
